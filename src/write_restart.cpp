...
#include "group.h"
#include "region.h" //NP modified C.K.
#include "force.h"
#include "pair.h"
#include "bond.h"
#include "angle.h"
...
void WriteRestart::command(int narg, char **arg)
{
  ...
  if (narg != 1 && narg != 3) error->all(FLERR,"Illegal write_restart command");  //NP modified C.K
  ...
  if ((ptr = strchr(arg[0],'*'))) { //NP modified R.B.
    *ptr = '\0';
    sprintf(file,"%s" BIGINT_FORMAT "%s",arg[0],update->ntimestep,ptr+1);
  } else strcpy(file,arg[0]);

  //NP modified C.K:
  //NP if two args, get region
  int iregion;
...
void WriteRestart::write(char *file)
{
  ...
  if (natoms != atom->natoms && output->thermo->lostflag == ERROR && !region) //NP modified C.K.
  ...
  else
  {
      ...
      //NP update send size, bc not all particles may be in region
      send_size = n;
  }
  ...
  if (modify->restart_pbc_any) {
    ...
    double *x;
    if(region) error->all(FLERR,"have to implement more here");//NP modified C.K.
    int m = 0;
    for (int i = 0; i < atom->nlocal; i++) {
      x = &buf[m+1];
